...
class Youtube extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, page, uri, links, date_create, description, visible', 'required'),
			array('visible', 'numerical', 'integerOnly'=>true),
			array('title, page, uri, links', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, page, uri, links, date_create, description, visible', 'safe', 'on'=>'search'),
		);
	}
